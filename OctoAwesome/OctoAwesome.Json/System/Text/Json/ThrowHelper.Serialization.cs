    ...
    internal static partial class ThrowHelper
    {
        ...
        public static void ThrowNotSupportedException_DeserializeNoConstructor(JsonTypeInfo typeInfo, ref Utf8JsonReader reader, scoped ref ReadStack state)
        {
            ...
            if (type.IsInterface || type.IsAbstract)
            {
                ...
                {
                    message = SR.Format("DeserializationMustSpecifyTypeDiscriminator {0}", type);
                }
                ...
                {
                    message = SR.Format("DeserializeInterfaceOrAbstractType {0}", type);
                }
            }
            else
            {
        ...
        {
            state.Current.JsonPropertyName = propertyName.ToArray();
            ThrowJsonException("MetadataIdCannotBeCombinedWithRef");
        }
        ...
        {
            ThrowJsonException(SR.Format("DuplicateMetadataProperty {0}", JsonHelpers.Utf8GetString(utf8PropertyName)));
        }
        ...
        {
            throw new OperationCanceledException("PipeWriterCanceled");
        }

        [DoesNotReturn]
        public static void ThrowOperationCanceledException_PipeWriteCompleted()
        {
            throw new OperationCanceledException("PipeWriterCompleted");
        }
    }
}
