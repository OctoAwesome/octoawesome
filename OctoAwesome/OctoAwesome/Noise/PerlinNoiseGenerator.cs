...
namespace OctoAwesome.Noise
{
    public class PerlinNoiseGenerator : INoise
    {
        ...
        public float SmoothedNoise3(int x, int y, int z)
        {
            ...
            float directSides = (Noise3(x + 1, y, z) + Noise3(x - 1, y, z) + Noise3(x, y + 1, z) + Noise3(x, y - 1, z) +
                                 Noise3(x, y, z - 1) + Noise3(x, y, z + 1)) / (6 * Smoothfactor);

            float indirectSides = (Noise3(x + 1, y + 1, z) + Noise3(x - 1, y - 1, z) + Noise3(x - 1, y + 1, z) +
                                   Noise3(x + 1, y - 1, z) +
        ...
        public float[,] PerlinNoise2(int startX, int startY, int width, int heigth)
        {
            ...
            for (int i = 0; i < Octaves; i++)
            {
                ...
                for (int x = 0; x < width; x++)
                {
                    ...
                    {
                        noiseLayers[i, x, y] =
                            InterpolatedSNoise2(((float)(x + startX) / Sizefactor) * frequency,
